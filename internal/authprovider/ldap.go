...
func (m *UserMemberships) FromUserDN(userDN string) error {
	var err error
	m.AdminAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.AdminGroupBase)
	if err != nil {
		return errors.Wrap(err, "error getting admin access")
	}

	m.AppOpsAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.AppMasterGroupBase)
	if err != nil {
		return errors.Wrap(err, "error getting app ops access")
	}

	m.CustomerOpsAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.CustomerOpsGroupBase)
	if err != nil {
		return errors.Wrap(err, "error getting customer ops access")
	}

	m.ViewerAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.ViewerGroupBase)
	if err != nil {
		return errors.Wrap(err, "error getting viewer access")
	}

	m.ServiceAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.ServiceGroupBase)
	if err != nil {
		return errors.Wrap(err, "error getting service access")
	}

	m.CloudOpsAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.OpsMasterGroupBase)
	if err != nil {
	...
	m.ClusterGroupsAccess, err = getGroupMemberships(userDN, utils.Config.Ldap.GroupBase)
	if err != nil {
...
func (m *UserMemberships) ToGroups() []string {
	var groups []string
...
func getGroupMemberships(userDN string, groupBaseDN string) ([]*ldap.Entry, error) {
...
// TODO evrardjp: Bring this function back to getGroupMemberships if not used elsewhere
...
// TODO evrardjp: remove newGroupSearchRequest.
